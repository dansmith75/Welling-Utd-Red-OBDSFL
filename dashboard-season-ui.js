...
(() => {
  ...
  function completed(match) {
    return ["Win", "Draw", "Loss"].includes(match?.result);
  }
  ...
  function horizontalBar(name, canvasId, labels, data, colour) {
    ...
    charts[name] = new Chart(canvas, {
      ...
      options: {
        ...
        layout: { padding: { right: 26, top: 8, bottom: 8 } },
        scales: {
  ...
  renderOverview = function () {
    ...
    const cards = [
      [stats.games, "Competitive Games", "all"],
      [stats.wins, "Wins", "win"],
      [stats.draws, "Draws", "draw"],
      [stats.losses, "Losses", "loss"],
      [stats.goalsFor, "Goals For", "gf"],
      [stats.goalsAgainst, "Goals Against", "ga"]
    ];
    container.innerHTML = cards.map(([value, label, key]) =>
      `<div class="stat drill" data-season-competitive="${key}"><b>${value}</b><span>${label}</span></div>`
    ).join("");

    container.querySelectorAll("[data-season-competitive]").forEach(card => {
  ...
  function setAttendanceMode(mode, render = true) {
    ...
    document.querySelectorAll(".attendance-mode").forEach(button => {
      button.classList.toggle("active", button.dataset.attendanceMode === attendanceMode);
    });
  ...
  renderAttendance = function () {
    ...
    const other = isMatch ? "trainingAttendance" : "matchAttendance";
    destroyChart(other);

    const labels = dashboardPlayers();
    const data = attendanceTotals(rows);
    if (mobile()) horizontalBar(chartName, canvasId, labels, data, colour);
    else drawBar(chartName, canvasId, labels, data, colour);
  };
  ...
  // Once a player is selected, show only that button. Tapping it again restores all players.
  ...
  function renderLinks() {
    const target = document.getElementById("usefulLinks");
    ...
    rows.forEach(row => {
      const category = row.category || "Useful Links";
    ...
    target.innerHTML = [...groups.entries()].map(([category, items]) => `
      <div class="link-group">
        ...
        <div class="useful-links-grid">
          ${items.map(item => `
            <a class="useful-link-card" href="${item.url}" target="_blank" rel="noopener noreferrer">
  ...
  function renderLeagueTable() {
    ...
    const empty = document.getElementById("leagueTableEmpty");
    if (!body || !empty) return;
    const rows = store.leagueTable || [];
    empty.style.display = rows.length ? "none" : "block";
    body.innerHTML = rows.map(row => {
      const ours = /welling united/i.test(String(row.team || ""));
      return `<tr class="${ours ? "our-team-row" : ""}">
        <td>${row.position ?? ""}</td><td>${row.team || ""}</td><td>${row.played ?? 0}</td><td>${row.won ?? 0}</td><td>${row.drawn ?? 0}</td><td>${row.lost ?? 0}</td><td>${row.goalsFor ?? 0}</td><td>${row.goalsAgainst ?? 0}</td><td>${row.goalDifference ?? 0}</td><td><strong>${row.points ?? 0}</strong></td>
      </tr>`;
    }).join("");
  }
  ...
  Promise.all([
    ...
    store.links = Array.isArray(links) ? links : [];
    store.leagueTable = Array.isArray(league) ? league : [];
    const activePage = document.querySelector(".page.active")?.id;
    if (activePage === "links") renderLinks();
    if (activePage === "league") renderLeagueTable();
  }).catch(() => {
    store.links = [];
    store.leagueTable = [];
  });
  ...
  style.textContent = `
    .attendance-toggle-card { padding:10px; margin-bottom:18px; }
    .attendance-toggle { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:8px; }
    .attendance-mode { border:1px solid var(--line); border-radius:12px; padding:11px 16px; background:transparent; color:var(--text); font-weight:bold; cursor:pointer; }
    .attendance-mode.active { background:linear-gradient(135deg,#1e3a8a,#2563eb); border-color:#2563eb; color:white; }
    .link-group + .link-group { margin-top:24px; }
    .useful-links-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:12px; }
    .useful-link-card { display:flex; flex-direction:column; gap:6px; padding:16px; border:1px solid var(--line); border-radius:14px; background:var(--card); color:var(--text); text-decoration:none; }
    .useful-link-card:hover { transform:translateY(-1px); border-color:#38bdf8; }
    .useful-link-card span { color:var(--muted); font-size:13px; line-height:1.4; }
    .our-team-row td { font-weight:bold; background:rgba(37,99,235,.14); }
    @media (max-width:650px) {
      #attendance .chart-scroll { overflow-x:visible; }
      #attendance .chart-box { width:100%; min-width:0 !important; }
      .useful-links-grid { grid-template-columns:1fr; }
      .league-table-wrap { border-right:2px solid rgba(56,189,248,.45); -webkit-overflow-scrolling:touch; }
    }
  `;
